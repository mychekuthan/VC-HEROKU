...
async def play(client: Client, message_: Message):
    ...
    if audio:
        ...
        if offset == None:
            await res.edit_text(" GIVE ME A YT LINK TO PLAY [☺️](https://telegra.ph/file/9ae661b212ecce3ecf1a1.jpg)")
            return
...
async def deezer(client: Client, message_: Message):
    ...
    queryy = text[1]
    res = await message_.reply_text(f"SEARCHING ⎳𝙀𝙇𝙄𝙕𝘼⏤͟͟͞͞★ʳᶱᵇ⁰ᶤ `{queryy}` ON DEEZER")
    try:
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        ...
        await res.edit_text("⎳𝙀𝙇𝙄𝙕𝘼⏤͟͟͞͞★ʳᶱᵇ⁰ᶤ PLAYING ON VC [☺️](https://telegra.ph/file/9ae661b212ecce3ecf1a1.jpg)")
        tgcalls.pytgcalls.join_group_call(message_.chat.id, file_path, 48000 , tgcalls.pytgcalls.get_cache_peer())
    await res.delete()
    m = await client.send_photo(
        chat_id=message_.chat.id,
...
async def jiosaavn(client: Client, message_: Message):
    ...
    text = message_.text.split(" ", 1)
    query = text[1]
    res = await message_.reply_text(f"⎳𝙀𝙇𝙄𝙕𝘼⏤͟͟͞͞★ʳᶱᵇ⁰ᶤ SEARCHING `{query}` ON JIO SAAVAN")
    try:
...
async def ytp(client: Client, message_: Message):
    ...
    if message_.chat.id in tgcalls.pytgcalls.active_calls:
        ...
        await res.edit_text("⎳𝙀𝙇𝙄𝙕𝘼⏤͟͟͞͞★ʳᶱᵇ⁰ᶤ PLAYING [☺️](https://telegra.ph/file/9ae661b212ecce3ecf1a1.jpg)")
        tgcalls.pytgcalls.join_group_call(message_.chat.id, file_path, 48000 , tgcalls.pytgcalls.get_cache_peer())
    await res.edit("Processing Thumbnail.")
